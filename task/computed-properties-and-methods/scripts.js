Vue.createApp({
  ...
  computed: {
    bookHeaders() {
      //   console.log(Object.keys(this.books[0]));
      const headerChoices = {
        title: "Title",
        isbn: "ISBN",
        author: "Author",
      ...
      Object.keys(this.books[0]).forEach((e) => {
        for (header in headerChoices) {
          if (header === e) {
            headerSelection.push(headerChoices[header]);
          }
        }
        return e.includes(Object.keys(headerChoices));
      });
    ...
    filterBooks() {
      return this.books.filter((book) => {
        if (book.publisher === "O'Reilly Media") {
          ...
          book.price = this.discount(price);
          return true;
        }
      });
    },
